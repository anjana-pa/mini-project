...
class Home_Class(View):
    template_name = "Homebutton/index.html"
...
class Project_Class(View):
    ...
    @method_decorator(login_required)
    def get(self, request, *args, **kwargs):
        form = new_project_form(project_status_choices=['Pending', 'Completed'],data=request.POST or None)
    ...
    def post(self,request,*args,**kwargs):
        if request.META.get('HTTP_X_REQUESTED_WITH') == 'XMLHttpRequest':
            form=new_project_form(project_status_choices=['ongoing', 'pending'], data=request.POST or None)
            if form.is_valid():
