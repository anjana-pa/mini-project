...
class new_project_form(forms.Form):
    ...
    startdate = forms.DateField(
        ...
        widget=forms.DateInput(attrs={
            ...
            'required': True,
            'type':'date',
             'min': datetime.now().strftime('%Y-%m-%d'),  # Set min attribute to today's date
    ...
    user_field = forms.ModelMultipleChoiceField(queryset=Users.objects.all().exclude(is_superuser=True), label="Select User",widget=forms.SelectMultiple(attrs={'class': 'form-select', 'id': 'userField'}),required=True)

    def __init__(self, project_status_choices, *args, **kwargs):
        super().__init__(*args, **kwargs)

        # Get a list of user IDs who are already assigned to projects that are not completed
        assigned_user_ids = ProjectTeams.objects.filter(PROJECT__status__in=project_status_choices).values_list('USER', flat=True)

        # Exclude users who are already assigned to projects that are not completed
        self.fields['user_field'].queryset = Users.objects.exclude(id__in=assigned_user_ids).exclude(is_superuser=True)
